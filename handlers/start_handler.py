...
async def main_menu(
    ...
    try:
        ...
        if (
            time_diff.seconds > 600 or "master_key" not in user_data
        ):  # Если время жизни пароля вышло или мастер кей не введен в сессии
            try:
                await bot.edit_message_text(
                    text=get_text("Введите мастер-пароль"),
                    chat_id=message.from_user.id,
                    message_id=user_data["bot_last_message"],
                )
            except (TelegramBadRequest, KeyError) as e:
...
@router.message(F.text, my_states.States.first_entering_master_key)
async def registration_entering_master_key(message: types.Message, state: FSMContext, bot: Bot):
    user_data = await state.get_data()
    if True:  # Если пароль соответствует стандартам
        await bot.edit_message_text(
            text="Повторите введенный пароль",
            chat_id=message.from_user.id,
            message_id=user_data["bot_last_message"],
        )
...
@router.message(F.text, my_states.States.confirm_master_key)
